...
async function performDiscordAuthorize(authUrl, baseCookie) {
    let currentUrl = authUrl;
    let cookieJar = baseCookie;
    const headersBase = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
        ...(DISCORD_AUTH_COOKIE ? { Cookie: DISCORD_AUTH_COOKIE } : {}),
        ...(DISCORD_TOKEN ? { Authorization: DISCORD_TOKEN } : {}),
    };
    ...
    for (let i = 0; i < 5; i++) {
        try {
            ...
            cookieJar = mergeCookies(cookieJar, resp.headers['set-cookie']);

            if (resp.status >= 300 && resp.status < 400 && resp.headers.location) {
                // Follow redirect manually
                currentUrl = resp.headers.location.startsWith('http')
                    ? resp.headers.location
                    : new URL(resp.headers.location, currentUrl).toString();
                continue;
            }

            // Finished (non-redirect)
            return { success: true, cookie: cookieJar };
        } catch (err) {
            console.log(`⚠️ Auto-authorize step gagal: ${err.message}`);
            break;
        }
    }
    return { success: false, cookie: cookieJar };
}
    ...
    try {
        ...
        if (!siteKey) {
            ...
            if (authLinkMatch) {
                ...
                console.error("❌ SiteKey not found. Halaman meminta authorize akun Discord. Mencoba otomatis...");

                // Coba authorize otomatis memakai cookie Discord bila tersedia
                if (DISCORD_AUTH_COOKIE || DISCORD_TOKEN) {
                    try {
                        console.log("🤖 Mencoba authorize otomatis memakai DISCORD_AUTH_COOKIE / DISCORD_TOKEN...");
                        const authResult = await performDiscordAuthorize(authUrl, activeCookie);
                        activeCookie = authResult.cookie;
                ...
                if (!siteKey) {
                    ...
                    console.error("❌ SiteKey tidak ditemukan meski sudah mencoba authorize otomatis. Pastikan DISCORD_TOKEN atau DISCORD_AUTH_COOKIE valid.");
                    return false;
                }
            }
